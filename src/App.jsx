...
function maxRaceTotal(raceId) {
  return 38 + (raceId >= TRIFECTA_START_RACE_ID ? TRIFECTA_EXACT_PTS : 0);
}
...
.result-stats{display:grid;grid-template-columns:repeat(auto-fit, minmax(150px, 1fr));gap:8px;margin-bottom:16px}
    ...
    <div className="directions">
      ...
      <div className="dir-section">
        ...
        <div className="dir-card" style={{marginTop:10}}>
          ...
          <div className="dir-card-body" style={{marginTop:6}}>
            Starting after the summer break (the race after Hungary), you can also predict the top 3 finishers: P1, P2, and P3. Get all three right in exact order for <strong>{TRIFECTA_EXACT_PTS} points</strong>. Get the right three drivers in any order (a "boxed" Trifecta) for <strong>{TRIFECTA_BOXED_PTS} points</strong>. This pick is completely optional — skip it and your other three picks score as normal.
          </div>
        </div>
        <div className="dir-card" style={{marginTop:10}}>
          <div className="dir-card-title">Maximum: 38 pts per race, 58 pts once the Trifecta is live</div>
          <div className="dir-card-body" style={{marginTop:6}}>
            25 (P10) + 10 (DNF1) + 3 (Constructor) = <strong>38 pts</strong> maximum per race. Add {TRIFECTA_EXACT_PTS} (Trifecta) for <strong>58 pts</strong> maximum in the {RACES.filter(r => !r.cancelled && r.id >= TRIFECTA_START_RACE_ID).length} races after the summer break.
          </div>
        </div>
      </div>
          ...
          {result ? (
            <>
              <div className="result-stats">
                ...
                {selectedRace >= TRIFECTA_START_RACE_ID && (
                  <div className="rs-box">
                ...
                <tbody>
                  {standings
                    ...
                    .map((s, i) => {
                      ...
                      const isPerfect = d.total === maxRaceTotal(selectedRace);
                      ...
                      return (
                        <tr key={s.player} style={{background: rowBg}}>
                          ...
                          <td style={{fontSize:12,color:"#888"}}>
                            ...
                            {d.trifectapts > 0 && <span className="badge hit" style={{marginLeft:4,fontSize:10}}>+{d.trifectapts}</span>}
                          </td>
...
function generateRaceExport({ race, result, standings, allPicks }) {
  ...
  const sharedCSS = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .result-cards{display:grid;grid-template-columns:repeat(auto-fit, minmax(180px, 1fr));gap:12px;margin-bottom:28px}
  ...
  const raceRowsHtml = raceRows.map((s, i) => {
    ...
    const pos = result.finishing_order.indexOf(d.p10) + 1;
    const trifectaCell = d.trifecta
      ? `${d.trifecta.p1} · ${d.trifecta.p2} · ${d.trifecta.p3}${d.trifectapts > 0 ? ` <span class="badge hit">+${d.trifectapts}</span>` : ""}`
      : "—";
    return `<tr>
      ...
      <td style="font-size:11px;color:#888">${trifectaCell}</td>
      <td class="pts-col"><span class="big-pts ${d.total === maxRaceTotal(race.id) ? "perfect-pts" : ""}">${d.total}</span></td>
    </tr>`;
  }).join("");
...
<div class="result-cards">
  ...
  ${race.id >= TRIFECTA_START_RACE_ID ? `<div class="rc"><div class="rc-label">🏆 Trifecta</div><div class="rc-val" style="font-size:20px">${result.finishing_order[0]} · ${result.finishing_order[1]} · ${result.finishing_order[2]}</div><div class="rc-sub">${TRIFECTA_EXACT_PTS}pts exact · ${TRIFECTA_BOXED_PTS}pts boxed</div></div>` : ""}
</div>
<div class="section-title">All Picks · ${race.name} GP</div>
<div class="table-wrap">
  <table>
    <thead><tr><th>#</th><th>Player</th><th>P10 Pick</th><th>DNF1</th><th>Constructor</th><th>Trifecta</th><th style="text-align:right">Pts</th></tr></thead>
  ...
  return (
    <div className="directions">
      ...
      <div className="dir-section">
        ...
        <div className="dir-card">
          <div className="dir-card-body">
            Use this tab when a player forgets to submit their picks before a race starts. Select the player and the race, enter their three picks, and save. Starting with the race after Hungary, you can also enter their optional Trifecta bonus pick (top 3 finishers) if they want it. You can also use this to correct a pick entered in error. If picks already exist for that player and race, a warning will appear before you overwrite them. Picks entered here are treated identically to player-submitted picks for scoring purposes.
          </div>
        </div>
      </div>
...
function CommissionerPicks({ allPicks, onSavePick, players }) {
  ...
  const handleSave = () => {
    ...
    const pick = { p10: form.p10, dnf1: form.dnf1, constructor: form.constructor };
    pick.trifecta = trifectaComplete && !trifectaDupes ? form.trifecta : null;
    onSavePick(selectedPlayer, selectedRace, pick);
    setSaved(true);
    setTimeout(() => setSaved(false), 3000);
  };
  ...
  return (
    <div>
      ...
      <div className="pick-form">
        ...
        {selectedRace >= TRIFECTA_START_RACE_ID && (
          <div style={{borderTop:"1px solid #1a1a1a",margin:"18px 0 16px",paddingTop:16}}>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:12,letterSpacing:"0.2em",color:"#e10600",textTransform:"uppercase",marginBottom:10}}>
              🏆 Trifecta (optional)
            </div>
            <div className="form-row">
              <label className="form-label">P1</label>
